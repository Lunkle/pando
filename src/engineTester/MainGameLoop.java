...
public class MainGameLoop {
	...
	public static void main(String[] args) {
		...
		TexturedModel oakTreeStage1Model = new TexturedModel(rawOakTreeStage1Model,
				new ModelTexture(Loader.loadTexture("oakTreeStage1")));
...
//		gen.makeDefaultFile();
		...
		for (int i = 0; i < 10; i++) {
			...
			float entityZ = 10 + random.nextFloat() * 50;
			float entityY = terrainData.getHeightByWorldCoords(entityX, entityZ);
			oaks.add(new Entity(oakTreeStage1Model, new Vector3f(entityX, entityY, entityZ), 0,
		...
		GUITexture gui = new GUITexture(Loader.loadTexture("grassy"), new Vector2f(0.5f, 0.5f),
...
//			System.out.println(picker.getCurrentRay());
			...
			centerSprout.setPosition(new Vector3f(
					(coords.y % 2) * Terrain.HEX_HALF_SQRT3 + coords.x * Terrain.HEX_SQRT3 + Terrain.HEX_HALF_SQRT3,
					terrainData.getHeightByHexCoords((int) coords.x, (int) coords.y),
					coords.y * 1.5f * Terrain.HEX_SIDE + Terrain.HEX_SIDE));
